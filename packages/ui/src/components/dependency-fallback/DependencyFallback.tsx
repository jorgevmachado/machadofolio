...
import { Text, Button, joinClass } from '@repo/ds';
...
export type DependencyFallbackButtonProps = Omit<CustomButtonProps, 'children'> & {
    label: string;
};
...
const DependencyFallback: React.FC<DependencyFallbackProps> = ({
    ...
    const textMessage = useMemo(() => {
        ...
        const defaultProps: TextProps = {
            ...customText,
            children: text ?? `No list of ${dependencyName} were found. Please create a ${dependencyName} before creating a ${resourceName}!!`
        };
        return defaultProps;
    ...
    const buttonProps = useMemo(() => {
        ...
        return {
            ...
            className: joinClass([ 'ui-dependency-fallback__button', button?.className && button.className])
        }
    },[button])
