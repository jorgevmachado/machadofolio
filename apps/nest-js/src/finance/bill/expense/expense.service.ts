...
import EXPENSE_LIST_DEVELOPMENT_JSON
  from '../../../../seeds/development/finance/expenses.json';
import EXPENSE_LIST_PRODUCTION_JSON
  from '../../../../seeds/production/finance/expenses.json';
import EXPENSE_LIST_STAGING_JSON
  from '../../../../seeds/staging/finance/expenses.json';
...
export class ExpenseService extends Service<Expense> {
    ...
    for (const persistExpenseDto of cleanPersistExpenseParamsBuilt) {
      ...
      if (existExpense) {
        ...
        for (const monthToAdd of persistExpenseDto.months ?? []) {
          ...
          if (!monthExists) {
            existExpense.months.push(monthToAdd as any);
          }
        }
  ...
  async upload(listBillExpenseToCreate: Array<BillExpenseToCreate>) {
    const expenses: Array<Expense> = [];
    for (const item of listBillExpenseToCreate) {
      const supplier = await this.supplierService.createToSheet(item.title) as Supplier;
      const builtExpense = new ExpenseConstructor({
        supplier,
        bill: item.bill,
        year: item.year,
        type: EExpenseType.VARIABLE,
        paid: item.paid,
        description: 'CREATE BY UPLOADS',
        instalment_number: 1,
      });
      const existExpense = await this.validateExistExpense(item.bill, builtExpense, false);
      if(existExpense) {
        existExpense.months = existExpense.months ?? [];
        for (const monthToAdd of item.months ?? []) {
          const monthExists = existExpense.months.some(
            (m) => m.code === monthToAdd.code
          );
          if (!monthExists) {
        ...
        const savedExpense = await this.save(builtExpense) as Expense;
        if ((item.months?.length ?? 0) > 0) {
          savedExpense.months = await this.monthService.persistList(item.months ?? [], { expense: savedExpense });
          const savedExpenseCalculated = this.business.calculate(savedExpense);
          const finalExpense = await this.save(savedExpenseCalculated);
          if (finalExpense) {
            expenses.push(finalExpense);
