...
const DefaultCustomSvg = ({x, y, svg, fill, type, width, height}: DefaultCustomSvgProps): React.ReactNode => {
    if(!svg) {
        return (
            <svg x={x} y={y} width={width} height={height} fill={fill} viewBox="0 0 1024 1024" data-testid={`ds-line-chart-customized-dot-default-${type}`}>
                {type === 'max' ? (
...
export default function CustomizedDot({
    ...
    return (<DefaultCustomSvg {...params} x={x} y={y} svg={customDot?.svg} type={value > maxValue ? 'max' : 'min'} />);
}